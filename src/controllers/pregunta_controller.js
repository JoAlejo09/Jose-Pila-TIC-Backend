...
const crearPregunta = async(req,res)=>{
    try {
        ...
        // VALIDAR RECURSO
        if(recursoFinal){
            const tiposUrl = [ "imagen", "video", "pdf", "enlace"];
            if( tiposUrl.includes(recursoFinal.tipo)
                &&
                !recursoFinal.url
            ){
                return res.status(400).json({
    ...
    try {
        const preguntas = await Pregunta.find()
            ...
            .populate({
                ...
                populate:{
                    path:"unidad",
                    select:"nombre materia nivelAcademico",
                    populate:{
                        path:"materia",
...
const actualizarPregunta = async(req,res)=>{
    try {
        ...
        const { enunciado, tipoPregunta, opciones, respuestaCorrecta, explicacion, materia, tema, nivelAcademico, nivelDificultad, recursoApoyo, estado} = req.body;

        const tipoFinal = tipoPregunta || pregunta.tipoPregunta;
        ...
        if( tipoFinal === "opcion_multiple"
            ...
            const respuestaFinal = respuestaCorrecta
                ||
        ...
        if(recursoApoyo !== undefined){
            if( recursoApoyo.tipo & 
                ...
                if( tiposUrl.includes(recursoApoyo.tipo)
                    &&
                    !recursoApoyo.url
                ){
                    return res.status(400).json({
